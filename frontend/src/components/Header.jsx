...
const Header = () => {
  ...
  return (
    <>
      ...
      <header className="w-full px-6 p-4 flex justify-between items-center border-b border-slate-200 dark:border-slate-800/60 backdrop-blur-md z-50 bg-white/80 dark:bg-transparent">
        <div className="tracking-tight text-xl font-bold flex items-center justify-center gap-2 cursor-pointer text-slate-900 dark:text-white">
        ...
        <nav className="flex items-center gap-4">
          ...
          {user ? (
            ...
            <div className="relative inline-block p-0.5 rounded-full overflow-hidden hover:scale-105 transition duration-300 active:scale-100 before:content-[''] before:absolute before:inset-0 before:bg-[conic-gradient(from_0deg,_#00F5FF,_#00F5FF30,_#00F5FF)] button-wrapper">
              <button onClick={openSignIn} className="cursor-pointer relative z-10 bg-white text-slate-900 dark:bg-gray-800 dark:text-white rounded-full px-8 py-3 font-medium text-sm">
                Log In
              </button>
            </div>
          )}
